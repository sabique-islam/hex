...
export async function dispatchSlideCommand<T extends Record<string, unknown>>(
  ...
  if (id === 'slide.command.delete-slide') {
    const pageId = (params as { pageId?: string } | undefined)?.pageId;
    return deleteSlide(pageId);
  }
  if (id === 'slide.mutation.insert-element') {
    return insertSlideElementMutation(
      params as { unitId?: string; pageId?: string; element?: Record<string, unknown> } | undefined,
    );
  }
  if (id === 'slide.mutation.insert-page') {
    return insertSlidePageMutation(
      params as { unitId?: string; page?: Record<string, unknown>; index?: number } | undefined,
    );
  }
...
// ─────────────────────────────────────────────── Univer 0.25 fallbacks
//
// Casual Slides was built against a forked @univerjs/slides-ui that
// registered `slide.mutation.*` commands. Upstream 0.25.0 omits them,
// so toolbar/layout/background inserts would silently no-op. These
// helpers write the snapshot directly, refresh the canvas via CanvasView,
// and wrap undo through withUndo().

async function getSlideRenderController(unitId: string) {
  const univer = getUniver();
  if (!univer) return null;
  try {
    ...
    const mod = await import('@univerjs/slides-ui');
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const SlideRenderController = (mod as any).SlideRenderController;
    return renderUnit.with(SlideRenderController) ?? null;
  } catch {
    return null;
  }
}

function insertSlideElementMutation(
  params?: { unitId?: string; pageId?: string; element?: Record<string, unknown> },
): boolean {
  const unitId = params?.unitId ?? getFocusedSlideUnitId();
  const pageId = params?.pageId;
  const element = params?.element;
  if (!unitId || !pageId || !element?.id) return false;
  return withUndo(() => {
    const univer = getUniver();
    if (!univer) return false;
    const instances = univer.__getInjector().get(IUniverInstanceService);
    ...
    if (!page) return false;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    page.pageElements[element.id as string] = element as any;
    model.incrementRev();
    model.setActivePage(page);
    try {
      const canvasView = univer.__getInjector().get(CanvasView);
      const sceneObject = canvasView.createObjectToPage(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        element as any,
        pageId,
        unitId,
      );
      if (sceneObject) canvasView.setObjectActiveByPage(sceneObject, pageId, unitId);
    } catch {
      /* canvas not ready — snapshot still updated */
    }
    return true;
  });
}

function insertSlidePageMutation(
  params?: { unitId?: string; page?: Record<string, unknown>; index?: number },
): boolean {
  const unitId = params?.unitId ?? getFocusedSlideUnitId();
  const page = params?.page;
  if (!unitId || !page?.id) return false;
  return withUndo(() => {
    const univer = getUniver();
    if (!univer) return false;
    const instances = univer.__getInjector().get(IUniverInstanceService);
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    pages[pageId] = page as any;
    const insertIdx =
      typeof params?.index === 'number' && params.index >= 0
        ? Math.min(params.index, order.length)
        : order.length;
    order.splice(insertIdx, 0, pageId);
    model.incrementRev();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    model.setActivePage(page as any);
    void (async () => {
      try {
        const ctrl = await getSlideRenderController(unitId);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (ctrl as any)?.createPageScene?.(pageId, page);
        const canvasView = univer.__getInjector().get(CanvasView);
        canvasView.activePage(pageId, unitId);
      } catch {
        /* render refresh best-effort */
      }
    })();
    return true;
  });
...
function updateSlidePageMutation(
  params?: { unitId?: string; pageId?: string; patch?: Record<string, unknown> },
): boolean {
  const unitId = params?.unitId ?? getFocusedSlideUnitId();
  const pageId = params?.pageId;
  const patch = params?.patch;
  if (!unitId || !pageId || !patch) return false;
  return withUndo(() => {
    const univer = getUniver();
    if (!univer) return false;
    const instances = univer.__getInjector().get(IUniverInstanceService);
...
export function deleteSlide(pageId?: string): boolean {
  return withUndo(() => _deleteSlide(pageId));
}

function _deleteSlide(pageId?: string): boolean {
  const univer = getUniver();
  if (!univer) return false;
  const instances = univer.__getInjector().get(IUniverInstanceService);
  const model = instances.getCurrentUnitOfType<SlideDataModel>(UniverInstanceType.UNIVER_SLIDE);
  ...
  const order = snap.body?.pageOrder;
  const pages = snap.body?.pages;
  if (!order || !pages || order.length <= 1) return false;
  const targetId = pageId ?? model.getActivePage()?.id;
  if (!targetId) return false;
  const idx = order.indexOf(targetId);
  if (idx < 0) return false;
  order.splice(idx, 1);
  delete pages[targetId];
  const nextId = order[Math.min(idx, order.length - 1)];
  const nextPage = pages[nextId];
  if (!nextPage) return false;
  model.incrementRev();
  model.setActivePage(nextPage);
  try {
    const canvasView = univer.__getInjector().get(CanvasView);
    canvasView.activePage(nextId, model.getUnitId());
  } catch {
    /* canvas not ready */
  }
