...
export interface TitleBarProps {
  /** Optional brand slot (e.g. Hex logo). Replaces default Casual Slides logo. */
  brand?: ReactNode;
  fileName: string;
  onFileNameChange: (next: string) => void;
  onOpen: () => void;
  onSave: () => void;
  /** Optional download/export handler (Hex uses this for .pptx download). */
  onDownload?: () => void;
...
export function TitleBar({
  ...
  return (
    <header className="cs-titlebar">
      <div className="cs-titlebar__brand" aria-label={t('titlebar.brand')}>
        {brand ?? (
          <img
            src="/brand.svg"
      ...
      <div className="cs-titlebar__actions">
        ...
        {onDownload ? (
          <button type="button" className="cs-btn cs-btn--accent" onClick={onDownload}>
            <Icon name="download" size={18} />
            <span>Download</span>
          </button>
        ) : null}
