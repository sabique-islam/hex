EditorToolbar from @casualoffice/docs. Use via `window.CasualOfficeDocs.EditorToolbar` (bundle loaded from the root `_ds_bundle.js`).

Sub-components: `EditorToolbar.TitleBar`, `EditorToolbar.Logo`, `EditorToolbar.DocumentName`, `EditorToolbar.MenuBar`, `EditorToolbar.TitleBarRight`, `EditorToolbar.FormattingBar`. See the DS docs for composition — e.g. items like `EditorToolbar.Item` go inside `<EditorToolbar>`; containers like `EditorToolbar.Group` wrap multiple `<EditorToolbar>`s.
...
`EditorToolbar.TitleBar`, `EditorToolbar.Logo`, `EditorToolbar.DocumentName`, `EditorToolbar.MenuBar`, `EditorToolbar.TitleBarRight`, `EditorToolbar.FormattingBar`
